import { createNews, createTranslation, setThumbnail, updateTranslation } from "@/lib/api/news";
...
export const useCreateNewsMutation = () => useMutation({
  mutationFn: async ({ data, thumbnail, attachments, locale }: createNewsOpts) => {
...
export const useUpdateNewsMutation = (newsId: string) => useMutation({
  mutationFn: async ({ locale, thumbnail, data, attachments }: updateNewsProps) => {
